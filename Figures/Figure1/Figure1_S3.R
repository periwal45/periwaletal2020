...
ggplot(fv_filter, aes(reorder(name, -feature_importance), feature_importance)) + geom_bar(stat = "identity", fill = "#bdbdbd") +
  xlab("Features (top 20)") + ylab("Feature importance") +
...
##Fig 1E
hyp<-data.frame(read.table(file="Figure1/Models_TrainingMeasures", header = TRUE, sep = '\t'))
head(hyp)
...
melt.hyp %>% dplyr::group_by(Model) %>% ggplot(aes(x=value,y=variable,fill=Model)) + 
  geom_boxplot(lwd=0.3) + scale_fill_jama() + scale_y_discrete(name = "Measure") + scale_x_continuous(name = "Value") +
