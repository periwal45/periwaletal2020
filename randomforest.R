...
# train a model
rforest_dist_desc <- train(rf.prob, train_task_dist_desc)
getLearnerModel(rforest_dist_desc)

# save the model
save(rforest_dist_desc, file = "rforest_default.rda")

### Optimization and training (hyp1)
...
### Optimization and training (hyp2)
...
### Optimization and training (hyp3)

# set tunable parameters

# 1. Specifying the search space of learner (RandomForest)
rf_param <- makeParamSet(
   makeDiscreteParam("wcw.weight", seq(1000,30000,5000)),
   makeIntegerParam("ntree", lower = 50, upper = 101),
   makeIntegerParam("nodesize", lower =30, upper = 150)
)
...
# train a model
rforest_highcost <- train(rf.tree_dist_desc, train_task_dist_desc)
getLearnerModel(rforest_highcost)

# save the model
save(rforest_highcost, file = "rforest_hyp3.rda")
